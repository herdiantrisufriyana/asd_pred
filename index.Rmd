...
```{r Neo risk model SHAP - show, echo=FALSE, fig.height=1.5, fig.width=7}
shap_beeswarm_plots$neonatal_risk[[selected_model$best_f1$neonatal_risk]]
```
...
cat(
  paste0(
    ...
    , "factors. Interval estimate of SHAP values are shown for higher feature "
    , "values (>= average). CI, confidence interval; OR, odds ratio; SHAP, "
...
```{r BSID-III model SHAP - show, echo=FALSE, fig.height=2.5, fig.width=7}
shap_beeswarm_plots$bsid_iii[[selected_model$best_f1$bsid_iii]]
```
...
cat(
  paste0(
    "Figure ", figure_n,". "
    , "SHAP beeswarm plot of best prediction model using BSID-III. Interval "
    , "estimate of SHAP values are shown for higher feature values "
...
```{r M-CHAT-R model SHAP - show, echo=FALSE, fig.height=2.5, fig.width=7}
shap_beeswarm_plots$m_chat_r[[selected_model$best_f1$m_chat_r]]
```
...
cat(
  paste0(
    "Figure ", figure_n,". "
    , "SHAP beeswarm plot of best prediction model using M-CHAT-R. Interval "
    , "estimate of SHAP values are shown for higher feature values "
...
cat(
  paste0(
    ...
    , "based on validation set. Performances using the selected threshold are "
    , "shown by dashed line in (A) and points in (B). ASD, autism spectrum "
    , "disorder; M-CHAT-F, modified checklist for autism in toddlers, "
    , "follow-up; DT, decision tree; RF, random forest; ROC, receiver "
...
shap_beeswarm_plots$m_chat_f[[selected_model$best_f1$m_chat_f]]
```
...
cat(
  paste0(
    "Figure ", figure_n,". "
    , "SHAP beeswarm plot of best prediction model using M-CHAT-F. Interval "
    , "estimate of SHAP values are shown for higher feature values "
...
cat(
  paste0(
    ...
    , "sensitivity/recall; M-CHAT-F, modified checklist for autism in "
    , "toddlers, follow-up; M-CHAT-R, modified checklist for autism in "
    , "toddlers, revised; N, neonatal risk factors; NB, neonatal risk factors "
    , "and BSID-III; NBR, neonatal risk factors, BSID-III, and M-CHAT-R; NBF, "
    , "neonatal risk factors, BSID-III, and M-CHAT-F; NPV, negative predictive "
    , "value or precision; PPV, positive predictive value or precision; TNR, "
    , "true negative rate or specificity; TPR, true positive rate or "
    , "sensitivity/recall."
  )
)
```
...
comb_step_prob <-
  ...
  lapply(\(x) imap(eval$df, ~ .x[[selected_model$best_f1[[.y]]]][[x]])) |>
...
comb_model_protocol <-
  list(
    expand.grid(step1 = paste0("m_chat_r=", 0:1), step2 = "m_chat_f")
    , expand.grid(
        step1 = paste0("bsid_iii=", 0:1)
        , step2 = paste0("m_chat_r=", 0:1)
        , step3 = "m_chat_f"
      )
    , expand.grid(
        step1 = paste0("neonatal_risk=", 0:1)
        , step2 = paste0("bsid_iii=", 0:1)
        , step3 = paste0("m_chat_r=", 0:1)
        , step4 = "m_chat_f"
      )
    , expand.grid(step1 = paste0("bsid_iii=", 0:1), step2 = "m_chat_r")
    , expand.grid(
        step1 = paste0("neonatal_risk=", 0:1)
        , step2 = paste0("bsid_iii=", 0:1)
        , step3 = "m_chat_r"
      )
    , expand.grid(step1 = paste0("neonatal_risk=", 0:1), step2 = "bsid_iii")
  )
...
```{r Prediction by either M-CHAT-R or M-CHAT-F only, include=FALSE}
m_chat_pred <-
  predmod_data$asd[c("m_chat_r", "m_chat_f")] |>
  imap(~ lapply(.x, select_at, c("id", "outcome", .y))) |>
  lapply(lapply, `colnames<-`, c("id", "obs", "pred")) |>
  lapply(lapply, mutate_at, "id", as.numeric) |>
  imap(~ lapply(.x, mutate, th = ifelse(.y == "m_chat_r", 3, 2))) |>
...
```{r Assess comb advantages - show, echo=FALSE, fig.height=4.5, fig.width=10}
comb_advantage
```
...
cat(
  paste0(
    ...
    , "development version III; CI, confidence interval; F, M-CHAT-F, F1, 2 x "
    , "precision x recall / (precision + recall); FNR, false negative rate or "
    , "specificity; FPR, false positive rate or sensitivity/recall; M-CHAT-F, "
    , "modified checklist for autism in toddlers, follow-up; M-CHAT-R, "
    , "modified checklist for autism in toddlers, revised; N, neonatal risk "
    , "factors; NB, neonatal risk factors and BSID-III; NBR, neonatal risk "
    , "factors, BSID-III, and M-CHAT-R; NBF, neonatal risk factors, BSID-III, "
    , "and M-CHAT-F; NPV, negative predictive value or precision; PPV, "
    , "positive predictive value or precision; R, M-CHAT-R; TNR, true negative "
    , "rate or specificity; TPR, true positive rate or sensitivity/recall."
  )
)
```

```{r Select best combination, include=FALSE}
selected_comb <- c("N0-NB0-NBR0-NBF", "N0-NB0-NBR")
```
...
eval_performance <-
  ...
  rbind(
    validation2_performance |>
      filter(Step %in% c(selected_comb, "Universal R", "Universal F"))
  ) |>
...
```{r Val-test diff in perform - show, echo=FALSE, fig.height=2.5, fig.width=10}
diff_eval_performance
```
...
workflow_prevalence_dag <-
  ggarrange(
    ggarrange(
      ...
      , workflow_prevalence_dag$`Universal NB`
      , workflow_prevalence_dag$`Universal NBR`
      , workflow_prevalence_dag$`Universal NBF`
      , nrow = 1, ncol = 4, labels = LETTERS[1:4]
    )
    , ggarrange(
      workflow_prevalence_dag$`N0-NB0-NBR0-NBF`
      , ggarrange(
          workflow_prevalence_dag$`N0-NB0-NBR`
          , ggarrange(
              workflow_prevalence_dag$`Universal R`
              , workflow_prevalence_dag$`Universal F`
              , nrow = 1, ncol = 2, labels = LETTERS[7:8]
            )
          , nrow = 2, ncol = 1, heights = c(5, 2.5)
          , labels = c(LETTERS[6], "")
        )
      , nrow = 1, ncol = 2, labels = c(LETTERS[5], "")
    )
    , nrow = 2, ncol = 1, heights = c(2.5, 7.5)
  )
```

```{r Workflow prevalence - show, echo=FALSE, fig.height=10, fig.width=10}
workflow_prevalence_dag
```
...
cat(
  paste0(
    "Figure ", figure_n,". "
    , "Workflow with prevalence: (A) Universal N; (B) Universal NB; (C) "
    , "Universal NBR; (D) Universal NBF; (E) N0-NB0-NBR0-NBF; (F) N0-NB0-NBR; "
    , "(G) Universal R; and (H) Universal F. BSID-III, Bayley scales of infant "
    , "development version III; F, M-CHAT-F; M-CHAT-F, modified checklist for "
    , "autism in toddlers, follow-up; M-CHAT-R, modified checklist for autism "
    , "in toddlers, revised; N, neonatal risk factors; NB, neonatal risk "
    , "factors and BSID-III; NBR, neonatal risk factors, BSID-III, and "
    , "M-CHAT-R; NBF, neonatal risk factors, BSID-III, and M-CHAT-F; R, "
    , "M-CHAT-R."
  )
)
```
