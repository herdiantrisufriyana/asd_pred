...
author: "Li-Wen Chen, Herdiantri Sufriyana, Chien-Jung Ho, Ting-Chun Yeh, Chao-Ching
...
modeval_step_empty <-
  ...
  unique() |>
  rbind(data.frame(variable = c("m_chat_r2f"))) |>
  mutate(step = NA) |>
  rbind(data.frame(variable = NA, step = c("only_bsid_iii")))
```

```{r Modeling step - write, eval=FALSE, include=FALSE}
modeval_step_empty |>
...
predictor <-
  list(
    asd =
      c(multivar_reg_sig_var$asd, "m_chat_r2f", "m_chat_rf_r", "m_chat_rf_f")
  ) |>
  ...
  lapply(
    ...
    , factor(
        step
        , c("neonatal_risk", "bsid_iii", "m_chat_r", "m_chat_f", "m_chat_r2f"
            , "m_chat_rf", "only_bsid_iii")
      )
  ) |>
  imap(
    ~ list(
        ...
        , c("neonatal_risk", "bsid_iii", "m_chat_r2f")
        , c("neonatal_risk", "bsid_iii", "m_chat_rf")
        , c("only_bsid_iii")
      ) |>
      `names<-`(
        c("neonatal_risk", "bsid_iii", "m_chat_r", "m_chat_f", "m_chat_r2f"
...
modeval_data <-
  processed_data |>
  mutate(
    m_chat_r2f = ifelse(m_chat_r < 3, 0, ifelse(m_chat_f < 2, 0, 1))
    , m_chat_rf_r = m_chat_r
    , m_chat_rf_f = m_chat_f
  ) |>
...
```{r Samp size M-CHAT-R/F - plot, include=FALSE}
sample_size_estimation_plot_m_chat_r2f <-
  sample_size_estimation_plot_data |>
  filter(step == "m_chat_r2f") |>
  ggplot(aes(epv, auc_roc)) +
  geom_smooth(method = "loess", formula = y ~ x) +
  geom_line(aes(y = auc_roc_med), color = "red", na.rm = TRUE) +
  geom_point() +
    ...
    data =
      ...
      mutate(
        ...
        , auc_roc =
          auc_roc
          - ifelse(
            algorithm %in% c("svm", "dnn")
            , ifelse(algorithm %in% "dnn", -0.3, -0.1)
            , 0.2
          )
      ) |>
      left_join(algorithms, by = join_by(algorithm)) |>
...
```{r Samp size M-CHAT-R/F - show, echo=FALSE, fig.height=5, fig.width=7}
sample_size_estimation_plot_m_chat_r2f
```

```{r Samp size M-CHAT-R/F - caption, echo=FALSE}
figure_n <- figure_n + 1
cat(
  paste0(
    "Figure ", figure_n,". "
    , "Sample size estimation at step of M-CHAT-R/F. Blue line indicates "
    , "the LOESS. Red line indicates the modified exponential decay fitting. "
    , "p(a) is the p-value of a where a is a parameter represents the factor "
    , "by which the exponential term was scaled in the model and a higher "
    , "value generally means a quicker initial increase. p(k) is the p-value "
...
```{r Samp size M-CHAT-RF - plot, include=FALSE}
sample_size_estimation_plot_m_chat_rf <-
  sample_size_estimation_plot_data |>
  filter(step == "m_chat_rf") |>
  ggplot(aes(epv, auc_roc)) +
  geom_smooth(method = "loess", formula = y ~ x) +
  geom_line(aes(y = auc_roc_med), color = "red", na.rm = TRUE) +
  geom_point() +
...
```{r Samp size M-CHAT-RF - show, echo=FALSE, fig.height=5, fig.width=7}
sample_size_estimation_plot_m_chat_rf
```

```{r Samp size M-CHAT-RF - caption, echo=FALSE}
figure_n <- figure_n + 1
cat(
  paste0(
    "Figure ", figure_n,". "
    , "Sample size estimation at step of M-CHAT-RF. Blue line indicates "
    , "the LOESS. Red line indicates the modified exponential decay fitting. "
    , "p(a) is the p-value of a where a is a parameter represents the factor "
    , "by which the exponential term was scaled in the model and a higher "
    , "value generally means a quicker initial increase. p(k) is the p-value "
...
cost_of_outcome_results <-
  ...
  mutate(
    step =
      case_when(
        step == "neonatal_risk" ~ "Universal N"
        , step == "bsid_iii" ~ "Universal NB"
        , step == "m_chat_r" ~ "Universal NBR"
        , step == "m_chat_f" ~ "Universal NBF"
        , step == "m_chat_r2f" ~ "Universal NBR/F"
        , step == "m_chat_rf" ~ "Universal NBRF"
        , step == "only_bsid_iii" ~ "Universal B"
      )
  ) |>
...
cost_of_outcome_results |>
  ...
  footnote(
    paste0(
      "BSID-III, Bayley scales of infant development version III; M-CHAT-F, "
      , "modified checklist for autism in toddlers, follow-up; M-CHAT-R, "
      , "modified checklist for autism in toddlers, revised; M-CHAT-R/F, "
      , "if M-CHAT-R ≥3 and M-CHAT-F ≥2, then 1, otherwise 0; N, neonatal risk "
      , "factors; NB, neonatal risk factors and BSID-III; NBR, neonatal risk "
      , "factors, BSID-III, and M-CHAT-R; NBF, neonatal risk factors, "
      , "BSID-III, and M-CHAT-F; NBR/F, neonatal risk factors, "
      , "BSID-III, and M-CHAT-R/F; NBRF, neonatal risk factors, "
      , "BSID-III, M-CHAT-R, and M-CHAT-F."
    )
  ) |>
  column_spec(
...
shap_values <-
  ...
  imap(
    ~ paste0(
        "inst/extdata/model/", .y, "/", .x, "/shap_values.csv"
      ) |>
      `names<-`(.x)
  ) |>
  lapply(lapply, read_csv, show_col_types = FALSE)
```

```{r Combine SHAP with features of the models, include=FALSE}
shap_feature_values <-
  shap_values |>
  lapply(lapply, mutate, seq = seq(n())) |>
  lapply(lapply, gather, feature, shap_value, -seq) |>
  imap(
    ~ .x |>
      lapply(
        left_join
        , predmod_data$asd[[.y]]$train |>
          select(-id, -outcome) |>
          mutate(seq = seq(n())) |>
...
```{r SHAP beeswarm plot, include=FALSE}
shap_beeswarm_plots <-
  shap_feature_values |>
  lapply(
    lapply
    , \(x)
      x |>
        left_join(
          cleaned_metadata |>
            select(feature = colname, label = english_term) |>
            unique() |>
            add_row(
              data.frame(
                feature = "m_chat_r2f"
                , label = "M-CHAT-R score ≥3 and M-CHAT-F score ≥2"
              )
            )|>
            add_row(
              data.frame(
                feature = c("m_chat_rf_r", "m_chat_rf_f")
                , label = c("M-CHAT-R score", "M-CHAT-F score")
              )
            )
          , by = join_by(feature)
        ) |>
...
cat(
  paste0(
    ...
    , "indicate the cost-aware threshold. ASD, autism spectrum disorder; "
    , "M-CHAT-R, modified checklist for autism in toddlers, revised; DT, "
...
cat(
  paste0(
    ...
    , "cost-aware threshold. ASD, autism spectrum disorder; M-CHAT-F, modified "
    , "checklist for autism in toddlers, follow-up; DT, decision tree; RF, "
    , "random forest."
  )
)
```

```{r M-CHAT-R/F model calibration - plot, include=FALSE}
m_chat_r2f_calibration_plots <-
  ggarrange(
    eval$calibration$m_chat_r2f$dt$validation |>
      arrange_calib_plot(th$m_chat_r2f)
    , eval$calibration$m_chat_r2f$rf$validation |>
      arrange_calib_plot(th$m_chat_r2f)
    , eval$calibration$m_chat_r2f$dnn$validation |>
      arrange_calib_plot(th$m_chat_r2f)
    , nrow = 1, ncol = 3, widths = c(5, 5, 5), labels = LETTERS[1:3]
  )
```

```{r M-CHAT-R/F model calib - show, echo=FALSE, fig.height=6, fig.width=15}
m_chat_r2f_calibration_plots
```

```{r M-CHAT-R/F model calibration - caption, echo=FALSE}
figure_n <- figure_n + 1
cat(
  paste0(
    "Figure ", figure_n,". "
    , "ASD prediction model calibration plots using M-CHAT-R/F based on "
    , "validation set: (A) DNN; (B) DT; (C) RF. Vertical dashed lines indicate "
    , "the cost-aware threshold. ASD, autism spectrum disorder; M-CHAT-F, "
    , "modified checklist for autism in toddlers, follow-up; M-CHAT-R, "
    , "modified checklist for autism in toddlers, revised; M-CHAT-R/F, if "
    , "M-CHAT-R ≥3 and M-CHAT-F ≥2, then 1, otherwise 0; DNN, deep neural "
    , "network; DT, decision tree; RF, random forest."
  )
)
```

```{r M-CHAT-RF model calibration - plot, include=FALSE}
m_chat_rf_calibration_plots <-
  ggarrange(
    eval$calibration$m_chat_rf$dt$validation |>
      arrange_calib_plot(th$m_chat_rf)
    , eval$calibration$m_chat_rf$rf$validation |>
      arrange_calib_plot(th$m_chat_rf)
    , nrow = 1, ncol = 2, widths = c(5, 5), labels = LETTERS[1:2]
  )
```

```{r M-CHAT-RF model calib - show, echo=FALSE, fig.height=6, fig.width=10}
m_chat_rf_calibration_plots
```

```{r M-CHAT-RF model calibration - caption, echo=FALSE}
figure_n <- figure_n + 1
cat(
  paste0(
    "Figure ", figure_n,". "
    , "ASD prediction model calibration plots using M-CHAT-RF based on "
    , "validation set: (A) DT; (B) RF. Vertical dashed lines indicate the "
    , "cost-aware threshold. ASD, autism spectrum disorder; M-CHAT-F, modified "
    , "checklist for autism in toddlers, follow-up; M-CHAT-R, modified "
    , "checklist for autism in toddlers, revised; M-CHAT-RF, both M-CHAT-R and "
    , "M-CHAT-F were used; DNN, deep neural network; DT, decision tree; RF, "
    , "random forest."
  )
)
```

```{r Select well-calibrated models, include=FALSE}
selected_model <-
  list(
    well_calibrated =
      eval$calibration |>
...
```{r Neo risk model SHAP - show, echo=FALSE, fig.height=2.5, fig.width=7}
ggarrange(
  plotlist =
    shap_beeswarm_plots$neonatal_risk[
      selected_model$well_calibrated$neonatal_risk
    ]
  , nrow = length(selected_model$well_calibrated$neonatal_risk), ncol = 1
  , labels = LETTERS[seq(length(selected_model$well_calibrated$neonatal_risk))]
)
```

```{r Neo risk model SHAP - caption, echo=FALSE}
figure_n <- figure_n + 1
cat(
  paste0(
    "Figure ", figure_n,". "
    , "SHAP beeswarm plot of best prediction model using neonatal risk "
    , "factors: "
    , mention_numbering(selected_model$well_calibrated$neonatal_risk)
    , "Interval estimate of SHAP values are shown for higher feature values "
    , "(>= average). CI, confidence interval; OR, odds ratio; SHAP, Shapley "
    , "additive explanation."
  )
)
```

```{r BSID-III model SHAP - show, echo=FALSE, fig.height=4.5, fig.width=7}
ggarrange(
  plotlist =
    shap_beeswarm_plots$bsid_iii[selected_model$well_calibrated$bsid_iii]
  , nrow = length(selected_model$well_calibrated$bsid_iii), ncol = 1
  , labels = LETTERS[seq(length(selected_model$well_calibrated$bsid_iii))]
)
```

```{r BSID-III model SHAP - caption, echo=FALSE}
figure_n <- figure_n + 1
cat(
  paste0(
    "Figure ", figure_n,". "
    , "SHAP beeswarm plot of best prediction model using BSID-III: "
    , mention_numbering(selected_model$well_calibrated$bsid_iii)
    , "Interval estimate of SHAP values are shown for higher feature values "
    , "(>= average). BSID-III, Bayley scales of infant development version "
    , "III; CI, confidence interval; OR, odds ratio; SHAP, Shapley additive "
    , "explanation."
  )
)
```

```{r M-CHAT-R model SHAP - show, echo=FALSE, fig.height=4.5, fig.width=7}
ggarrange(
  plotlist =
    shap_beeswarm_plots$m_chat_r[selected_model$well_calibrated$m_chat_r]
  , nrow = length(selected_model$well_calibrated$m_chat_r), ncol = 1
  , labels =
    LETTERS[seq(length(selected_model$well_calibrated$m_chat_r))]
)
```

```{r M-CHAT-R model SHAP - caption, echo=FALSE}
figure_n <- figure_n + 1
cat(
  paste0(
    "Figure ", figure_n,". "
    , "SHAP beeswarm plot of best prediction model using M-CHAT-R: "
    , mention_numbering(selected_model$well_calibrated$m_chat_r)
    , "Interval estimate of SHAP values are shown for higher feature values "
    , "(>= average). CI, confidence interval; M-CHAT-R, modified checklist for "
    , "autism in toddlers, revised; OR, odds ratio; SHAP, Shapley additive "
    , "explanation."
  )
)
```

```{r M-CHAT-F model SHAP - show, echo=FALSE, fig.height=2.5, fig.width=7}
ggarrange(
  plotlist =
    shap_beeswarm_plots$m_chat_f[selected_model$well_calibrated$m_chat_f]
  , nrow = length(selected_model$well_calibrated$m_chat_f), ncol = 1
  , labels = LETTERS[seq(selected_model$well_calibrated$m_chat_f)]
)
```

```{r M-CHAT-F model SHAP - caption, echo=FALSE}
figure_n <- figure_n + 1
cat(
  paste0(
    "Figure ", figure_n,". "
    , "SHAP beeswarm plot of best prediction model using M-CHAT-F: "
    , mention_numbering(selected_model$well_calibrated$m_chat_f)
    , "Interval estimate of SHAP values are shown for higher feature values "
    , "(>= average). CI, confidence interval; M-CHAT-F, modified checklist for "
    , "autism in toddlers, follow-up; OR, odds ratio; SHAP, Shapley additive "
    , "explanation."
  )
)
```

```{r M-CHAT-R/F model SHAP - show, echo=FALSE, fig.height=6.5, fig.width=7}
ggarrange(
  plotlist =
    shap_beeswarm_plots$m_chat_r2f[selected_model$well_calibrated$m_chat_r2f]
  , nrow = length(selected_model$well_calibrated$m_chat_r2f), ncol = 1
  , labels = LETTERS[seq(length(selected_model$well_calibrated$m_chat_r2f))]
)
```

```{r M-CHAT-R/F model SHAP - caption, echo=FALSE}
figure_n <- figure_n + 1
cat(
  paste0(
    "Figure ", figure_n,". "
    , "SHAP beeswarm plot of best prediction model using M-CHAT-R/F: "
    , mention_numbering(selected_model$well_calibrated$m_chat_r2f)
    , "Interval estimate of SHAP values are shown for higher feature values "
    , "(>= average). CI, confidence interval; M-CHAT-F, modified checklist for "
    , "autism in toddlers, follow-up; M-CHAT-R, modified checklist for autism "
    , "in toddlers, revised; M-CHAT-R/F, if M-CHAT-R ≥3 and M-CHAT-F ≥2, then "
    , "1, otherwise 0; DNN, deep neural network; OR, odds ratio; SHAP, Shapley "
    , "additive explanation."
  )
)
```

```{r M-CHAT-RF model SHAP - show, echo=FALSE, fig.height=2.5, fig.width=7}
ggarrange(
  plotlist =
    shap_beeswarm_plots$m_chat_rf[selected_model$well_calibrated$m_chat_rf]
  , nrow = length(selected_model$well_calibrated$m_chat_rf), ncol = 1
  , labels = LETTERS[seq(length(selected_model$well_calibrated$m_chat_rf))]
)
```

```{r M-CHAT-RF model SHAP - caption, echo=FALSE}
figure_n <- figure_n + 1
cat(
  paste0(
    "Figure ", figure_n,". "
    , "SHAP beeswarm plot of best prediction model using M-CHAT-RF: "
    , mention_numbering(selected_model$well_calibrated$m_chat_rf)
    , "Interval estimate of SHAP values are shown for higher feature values "
    , "(>= average). CI, confidence interval; M-CHAT-F, modified checklist for "
    , "autism in toddlers, follow-up; M-CHAT-R, modified checklist for autism "
    , "in toddlers, revised; M-CHAT-RF, both M-CHAT-R and M-CHAT-F were used; "
    , "DNN, deep neural network; OR, odds ratio; SHAP, Shapley additive "
    , "explanation."
  )
)
```

```{r Universal B model SHAP - show, echo=FALSE, fig.height=5.5, fig.width=7}
ggarrange(
  plotlist =
    shap_beeswarm_plots$only_bsid_iii[
      selected_model$well_calibrated$only_bsid_iii
    ]
  , nrow = length(selected_model$well_calibrated$only_bsid_iii), ncol = 1
  , labels = LETTERS[seq(length(selected_model$well_calibrated$only_bsid_iii))]
)
```

```{r Universal B model SHAP - caption, echo=FALSE}
figure_n <- figure_n + 1
cat(
  paste0(
    "Figure ", figure_n,". "
    , "SHAP beeswarm plot of best prediction model using only BSID-III: "
    , mention_numbering(selected_model$well_calibrated$only_bsid_iii)
    , "Interval estimate of SHAP values are shown for higher feature values "
    , "(>= average). CI, confidence interval; M-CHAT-F, modified checklist for "
    , "autism in toddlers, follow-up; M-CHAT-R, modified checklist for autism "
    , "in toddlers, revised; M-CHAT-RF, both M-CHAT-R and M-CHAT-F were used; "
...
```{r Obtain multivariate regression effect direction, include=FALSE}
multivar_reg_direction <-
  multivar_reg  |>
  filter(outcome == "asd") |>
  filter(term != "(Intercept)") |>
  inner_join(
...
multivar_reg_direction <-
  multivar_reg_direction |>
  rbind(
    filter(multivar_reg_direction, feature == "m_chat_f") |>
      mutate(feature = ifelse(feature == "m_chat_f", "m_chat_r2f", feature))
    , filter(multivar_reg_direction, feature == "m_chat_r") |>
      mutate(feature = ifelse(feature == "m_chat_r", "m_chat_rf_r", feature))
    , filter(multivar_reg_direction, feature == "m_chat_f") |>
      mutate(feature = ifelse(feature == "m_chat_f", "m_chat_rf_f", feature))
  ) |>
  left_join(
    cleaned_metadata |>
      select(feature = colname, label = english_term) |>
  ...
  mutate(
    reg_direction =
      ifelse(
        OR >= 1 & LB > 1
        , "Positive", ifelse(OR < 1 & UB < 1, "Negative", "Insignificant")
      )
  ) |>
  select(feature, OR, reg_direction)
```

```{r Obtain SHAP beeswarm direction, include=FALSE}
shap_beeswarm_direction <-
  shap_beeswarm_plots |>
  lapply(lapply, \(x) x$data) |>
  lapply(
    lapply
    , mutate
    , shap_direction =
      ifelse(
        (shap_value >= 0 & feature_value > 0.5)
        | (shap_value < 0 & feature_value <= 0.5)
        , "Positive"
        , ifelse(
            (shap_value >= 0 & feature_value <= 0.5)
  ...
  lapply(lapply, group_by, feature, shap_direction) |>
  lapply(lapply, summarize, n = n(), .groups = "drop")|>
  lapply(lapply, spread, shap_direction, n, fill = 0) |>
  lapply(lapply, mutate, PNR = Positive / Negative) |>
  lapply(
    lapply
    , mutate
    , shap_direction =
      ifelse(PNR > 1, "Positive", ifelse(PNR < 1, "Negative", "Insignificant"))
  ) |>
  lapply(lapply, select, feature, PNR, shap_direction)
```

```{r Clinical assessment - empty, eval=FALSE, include=FALSE}
shap_beeswarm_plots |>
  imap(~ .x[selected_model$well_calibrated[[.y]]]) |>
  lapply(lapply, \(x) x$data) |>
  lapply(lapply, filter, shap_value != 0) |>
  lapply(lapply, arrange, desc(feature_num)) |>
  lapply(lapply, select, feature) |>
  lapply(lapply, unique) |>
  lapply(imap, ~ mutate(.x, algorithm = .y)) |>
  imap(~ mutate(reduce(.x, rbind), step = .y)) |>
  reduce(rbind) |>
  mutate(aligned = NA) |>
  select(step, algorithm, everything()) |>
  write_csv("inst/extdata/clinical_assessment_empty.csv")
```

```{r Clinical assessment - filled, include=FALSE}
clinical_assessment <-
  read_csv(
    "inst/extdata/clinical_assessment_filled.csv", show_col_types = FALSE
  )
```

```{r SHAP alignment with MVR - plot, include=FALSE}
shap_beeswarm_alignment <-
  shap_beeswarm_direction |>
  imap(~ .x[selected_model$well_calibrated[[.y]]]) |>
  lapply(imap, ~ mutate(.x, algorithm = .y)) |>
  imap(~ mutate(reduce(.x, rbind), step = .y)) |>
  reduce(rbind) |>
  left_join(multivar_reg_direction, by = join_by(feature)) |>
  mutate(aligned = ifelse(shap_direction == reg_direction, "*", "")) |>
  left_join(clinical_assessment, by = join_by(step, algorithm, feature)) |>
  mutate(
    aligned.y =
      case_when(
        aligned.y == "yes" ~ "†"
  ...
  filter(aligned.y != "§") |>
  unite(aligned, aligned.x, aligned.y, sep = "") |>
  select(step, algorithm, feature, PNR, direction = shap_direction, aligned) |>
  arrange(factor(step, unique(names(shap_beeswarm_direction)))) |>
  mutate(
    step =
      case_when(
        step == "neonatal_risk" ~ "Universal N"
    ...
    , feature = str_replace_all(feature, "M chat ", "M-CHAT-")
  ) |>
  mutate_at("algorithm", str_to_upper) |>
  mutate_at(c("step", "algorithm"), \(x) factor(x, unique(x))) |>
  mutate_at("feature", \(x) factor(x, rev(unique(x)))) |>
  ggplot(aes(algorithm, feature, fill = direction)) +
  geom_tile(color = "white") +
  geom_text(aes(label = aligned)) +
  facet_grid(~ step, scales = "free", space = "free", switch = "x") +
  xlab("Model") +
  ylab("Feature") +
  scale_fill_discrete("Impact direction") +
  theme(legend.position = "top", strip.placement = "outside")
```

```{r SHAP alignment with MVR - show, echo=FALSE, fig.height=5, fig.width=14}
shap_beeswarm_alignment
```

```{r SHAP alignment with MVR - caption, echo=FALSE}
figure_n <- figure_n + 1
cat(
  paste0(
    "Figure ", figure_n,". "
    , "Feature impact alignment with multivariate regression analysis. "
    , "*, aligned according to positive-to-negative ratio; "
    , "†, aligned according to clinician assessment; "
    , "‡, unclear alignment according to clinician assessment; "
    , "Bayley scales of infant development version III; "
    , "M-CHAT-F, modified checklist for autism in toddlers, follow-up; "
    , "M-CHAT-R, modified checklist for autism in toddlers, revised; "
    , "M-CHAT-R/F, if M-CHAT-R ≥3 and M-CHAT-F ≥2, then 1, otherwise 0; "
    , "M-CHAT-RF, both M-CHAT-R and M-CHAT-F were used; "
    , "N, neonatal risk factors; "
    , "NB, neonatal risk factors and BSID-III; "
    , "NBR, neonatal risk factors, BSID-III, and M-CHAT-R; "
    , "NBF, neonatal risk factors, BSID-III, and M-CHAT-F; "
    , "NBR/F, neonatal risk factors, BSID-III, and M-CHAT-R/F; "
    , "NBRF, neonatal risk factors, BSID-III, and M-CHAT-RF."
  )
)
```

```{r Select well-calibrated interpretable best models, include=FALSE}
selected_model <-
  list(
    well_calibrated =
      eval$calibration |>
...
selected_model$interpretable <-
  shap_beeswarm_alignment$data |>
  filter(
    (feature == "Male" & aligned == "*†")
    | (step == "Universal B" & algorithm == "GBM")
  ) |>
  select(step, algorithm) |>
  unique()

selected_model$interpretable <-
  names(selected_model$well_calibrated) |>
  `names<-`(names(selected_model$well_calibrated)) |>
  lapply(
    \(x)
    selected_model$interpretable |>
      mutate(
        step2 =
          case_when(
            x == "neonatal_risk" ~ "Universal N"
            , x == "bsid_iii" ~ "Universal NB"
            , x == "m_chat_r" ~ "Universal NBR"
            , x == "m_chat_f" ~ "Universal NBF"
            , x == "m_chat_r2f" ~ "Universal NBR/F"
            , x == "m_chat_rf" ~ "Universal NBRF"
            , x == "only_bsid_iii" ~ "Universal B"
          )
      ) |>
      filter(step == step2) |>
      mutate_at("algorithm", \(x) str_to_lower(as.character(x))) |>
      pull(algorithm)
  )
  ...
  c(list(
      best_f1 =
        ...
        imap(
          ~ .x[
              intersect(
                selected_model$well_calibrated[[.y]]
                , selected_model$interpretable[[.y]]
              )
            ]
        ) |>
...
```{r Neo risk model DCA and ROC - plot, include=FALSE}
          ...
          list(
            data =
              eval$decision$neonatal_risk[
                selected_model$interpretable$neonatal_risk
              ] |>
              imap(
...
```{r Neo risk model DCA and ROC - best, echo=FALSE}
cat(
  "Best model:"
...
```{r Neo risk model DCA and ROC - caption, echo=FALSE}
figure_n <- figure_n + 1
cat(
  paste0(
    "Figure ", figure_n,". "
          ...
          list(
            data =
              eval$decision$m_chat_r[
                selected_model$interpretable$m_chat_r
              ] |>
              imap(
                ~ mutate(.x$validation$plot$data, model = str_to_upper(.y))
              ) |>
...
cat(
  paste0(
    ...
    , "disorder; M-CHAT-R, modified checklist for autism in toddlers, revised; "
    , "DT, decision tree; GBM, gradient boosting machine; RF, random "
    , "forest; ROC, receiver operating characteristics."
  )
)
```

```{r Universal B model DCA and ROC - plot, include=FALSE}
only_bsid_iii_dca_roc_curves <-
  ggarrange(
    list(
        plot =
          list(
            data =
              eval$decision$only_bsid_iii[
                selected_model$interpretable$only_bsid_iii
              ] |>
              imap(
                ~ mutate(.x$validation$plot$data, model = str_to_upper(.y))
              ) |>
      ...
      geom_vline(xintercept = th$only_bsid_iii, lty = 2)
    , list(
        plot =
          list(
            data =
              eval$discrimination$only_bsid_iii[
                selected_model$interpretable$only_bsid_iii
              ] |>
              imap(
                ~ mutate(.x$validation$plot$data, model = str_to_upper(.y))
      ...
      geom_point(
        data =
          ...
          reduce(rbind) |>
          rename(th2 = th) |>
          filter(th2 == th$only_bsid_iii)
      )
    , nrow = 1, ncol = 2, widths = c(5, 5), labels = LETTERS[1:2]
  )
```

```{r Universal B model DCA and ROC - best, echo=FALSE}
cat(
  "Best model:"
  , selected_model$best_f1$only_bsid_iii |>
    str_to_upper()
)
```

```{r Universal B model DCA and ROC - show, echo=FALSE, fig.height=4, fig.width=10}
only_bsid_iii_dca_roc_curves
```

```{r Universal B model DCA and ROC - caption, echo=FALSE}
figure_n <- figure_n + 1
cat(
  paste0(
    "Figure ", figure_n,". "
    , "ASD prediction model decision (A) and ROC (B) curves using only BSID-III "
    , "based on validation set. Performances using the selected threshold are "
    , "shown by dashed line in (A) and points in (B). ASD, autism spectrum "
    , "disorder; BSID-III, Bayley scales of infant development version III; "
...
validation_auroc <-
  eval$discrimination[
    names(selected_model$best_f1)[
      sapply(selected_model$best_f1, \(x) length(x) > 0)
    ]
  ] |>
  imap(~ .x[selected_model$best_f1[[.y]]]) |>
...
```{r Validation performances - show, echo=FALSE}
table_n <- table_n + 1
...
validation_performance |>
  ...
  footnote(
    paste0(
      ...
      , "BSID-III, Bayley scales of infant development version III; "
...
cat(
  paste0(
    ...
    , "operating characteristics; ASD, autism spectrum disorder; BSID-III, "
    , "Bayley scales of infant development version III; CI, "
    ...
    , "sensitivity/recall; M-CHAT-R, modified checklist for autism in "
    , "toddlers, revised; N, neonatal risk factors; NBR, neonatal risk "
    , "factors, BSID-III, and M-CHAT-R; NPV, negative predictive value or "
    , "precision; PPV, positive predictive value or precision; TNR, true "
    , "negative rate or specificity; TPR, true positive rate or "
    , "sensitivity/recall."
  )
)
```
...
```{r Combine probabilities from all the steps, include=FALSE}
comb_step_prob <-
  names(eval$df[[1]][[1]]) |>
  `names<-`(names(eval$df[[1]][[1]])) |>
  lapply(
    \(x)
    eval$df[
        names(selected_model$best_f1)[
      ...
      imap(~ .x[[selected_model$best_f1[[.y]]]][[x]])
  ) |>
  lapply(\(x) imap(x, ~ rename_at(.x, "pred", \(x) .y))) |>
  lapply(reduce, left_join, by = join_by(id, obs))
```

```{r Obtain prediction from all the steps, include=FALSE}
comb_step_pred <-
  comb_step_prob |>
  lapply(gather, step, prob, -id, -obs) |>
  lapply(mutate_at, c("id", "step"), \(x) factor(x, unique(x))) |>
  lapply(
    mutate
    , th = sapply(step, \(x) th[[x]])
    , pred =
      factor(
        ifelse(prob >= th, levels(obs)[2], levels(obs)[1])
        , levels(obs)
      )
  ) |>
  lapply(select, -prob, -th) |>
  lapply(spread, step, pred) |>
  lapply(arrange, id) |>
  lapply(mutate_at, "id", as.character) |>
  lapply(mutate_at, "id", as.numeric)
```

```{r Define protocol for combining models, include=FALSE}
comb_model_protocol <-
  list(
    expand.grid(
        step1 = paste0("neonatal_risk=", 0:1)
        , step2 = "only_bsid_iii"
      )
    , expand.grid(
        step1 = paste0("neonatal_risk=", 0:1)
        , step2 = paste0("only_bsid_iii=", 0:1)
        , step3 = "m_chat_r"
      )
  )
...
comb_model_protocol <-
  comb_model_protocol |>
  `names<-`(seq(length(comb_model_protocol))) |>
  imap(~ mutate(.x, type = .y)) |>
  lapply(mutate, model = seq(n())) |>
  lapply(mutate_all, as.character) |>
  lapply(gather, step, value, -type, -model) |>
  lapply(mutate, step = as.numeric(str_remove_all(step, "step"))) |>
  lapply(mutate, max_step = max(step)) |>
  reduce(rbind) |>
  select(type, max_step, everything())

comb_model_protocol <-
  comb_model_protocol |>
  left_join(
    comb_model_protocol |>
      select(type, max_step, model) |>
      unique() |>
      mutate(model2 = seq(n()))
    , by = join_by(type, max_step, model)
  ) |>
  mutate(model = model2) |>
  select(-type, -max_step, -model2) |>
  arrange(model)
  ...
  left_join(
    comb_model_protocol |>
      mutate(
        name =
          value |>
          str_replace_all("neonatal_risk", "N") |>
          str_replace_all("bsid_iii", "NB") |>
          str_replace_all("m_chat_r", "NBR") |>
          str_replace_all("m_chat_f", "NBF") |>
          str_replace_all("NBR2f", "NBR/F") |>
          str_replace_all("NBRf", "NBRF") |>
          str_replace_all("only_NB", "B") |>
          str_remove_all("\\=")
      ) |>
      group_by(model) |>
...
```{r Obtain prediction for each combined model, include=FALSE}
comb_model <-
  comb_step_pred |>
  lapply(obtain_obs_pred_comb, comb_model_protocol)

comb_model <-
  names(comb_model[[1]]) |>
  `names<-`(names(comb_model[[1]])) |>
  lapply(
    \(x)
    names(comb_model) |>
      `names<-`(names(comb_model)) |>
      lapply(
        \(y)
        comb_model[[y]][[x]]
      )
...
```{r Prediction by M-CHAT-R or M-CHAT-F or M-CHAT-R/F only, include=FALSE}
m_chat_pred <-
  predmod_data$asd[c("m_chat_r")] |>
  imap(~ lapply(.x, select_at, c("id", "outcome", .y))) |>
  lapply(lapply, `colnames<-`, c("id", "obs", "pred")) |>
  lapply(lapply, mutate_at, "id", as.numeric) |>
  imap(
    ~ .x |>
      lapply(
        mutate, th = ifelse(.y == "m_chat_r", 3, ifelse(.y == "m_chat_f", 2, 1))
      )
  ) |>
  lapply(lapply, mutate, pred = as.integer(pred >= th)) |>
  lapply(lapply, select, -th) |>
  lapply(lapply, mutate_at, c("obs", "pred"), as.factor)

m_chat_pred <- `names<-`(m_chat_pred, paste0("single_", names(m_chat_pred)))
```

# Model evaluation 2

```{r Create empty list for model evaluation 2, include=FALSE}
eval2 <- list()
```

```{r Obtain the model evaluation 2 data, include=FALSE}
eval2$df <-
  comb_model |>
  lapply(lapply, select, -step) |>
  c(m_chat_pred)
```

```{r Obtain the model evaluation 2 confusion matrix, include=FALSE}
eval2$threshold <-
  eval2$df |>
  lapply(lapply, select, -id) |>
  lapply(lapply, mutate, pred = as.numeric(as.character(pred))) |>
  pblapply(
    lapply
    , thresholding
    , standard = FALSE, optimal = FALSE, clinical = FALSE
    , custom_metric = "th", custom_ref = 1
    , seed = seed
  )
```

```{r Validation 2 confusion matrix, include=FALSE}
validation2_cm <-
  eval2$threshold |>
  imap(
    ~ .x$validation |>
      mutate(
        step = .y
        , algorithm = ifelse(str_detect(.y, "^single"), "none", "mixed")
      )
  ) |>
  reduce(rbind) |>
  select(step, algorithm, th = ref_value, everything()) |>
...
```{r Validation 2 performances - create, include=FALSE}
validation2_performance <-
  validation2_cm |>
  select(-th, -nb) |>
  mutate(
    algorithm =
      factor(
        str_to_sentence(algorithm)
        , c(
            str_to_upper(algorithms$algorithm)
            , str_to_sentence(unique(algorithm))
          )
      )
  ) |>
  mutate_at("step", as.character) |>
  mutate(
    step =
      ifelse(
        str_detect(step, "^single_")
        , str_to_upper(str_replace_all(step, "_", "-")) |>
          str_replace_all("SINGLE-M-CHAT-", "Universal ") |>
          str_replace_all("R2F", "R/F")
        , step
      )
  ) |>
  mutate(step = ifelse(duplicated(step), "", step))
...
validation2_performance <-
  validation2_performance |>
  rename_if(
    str_detect(colnames(validation2_performance), "step|algorithm")
    , str_to_sentence
  ) |>
  rename_if(
    !str_detect(colnames(validation2_performance), "step|algorithm")
...
```{r Validation 2 performance - show, echo=FALSE}
table_n <- table_n + 1

validation2_performance |>
  kable(
    caption = paste0("Table ", table_n, ". Validation 2 results (95% CI).")
    , format = kable_format
  ) |>
  footnote(
    paste0(
...
```{r Compute cost of step using study data, include=FALSE}
cost_of_step_data <-
  eval$df[
    names(selected_model$best_f1)[
      sapply(selected_model$best_f1, \(x) length(x) > 0)
  ...
  imap(~ .x[[selected_model$best_f1[[.y]]]]) |>
  imap(~ lapply(.x, mutate, pred = factor(as.integer(pred >= th[[.y]])))) |>
  lapply(
    lapply
    , mutate
    , variable =
      case_when(
        obs  ==  levels(obs)[2] & pred == levels(obs)[2] ~ "CTP"
        , obs == levels(obs)[2] & pred == levels(obs)[1] ~ "CFN"
        , obs == levels(obs)[1] & pred == levels(obs)[2] ~ "CFP"
        , obs == levels(obs)[1] & pred == levels(obs)[1] ~ "CTN"
      )
  ) |>
  imap(~ lapply(.x, mutate, step = .y)) |>
  lapply(
    lapply
    , left_join
    , select(cost_of_outcome, step, variable, cost = answer)
    , by = join_by(step, variable)
  ) |>
  lapply(lapply, group_by, variable) |>
  lapply(lapply, summarize, n = n(), cost = sum(cost)) |>
  lapply(lapply, mutate, p = n / sum(n)) |>
  imap(~ lapply(.x, mutate, step = .y)) |>
  lapply(lapply, select, step, everything())
```

```{r Compute cost of comb using study data, include=FALSE}
cost_of_comb_data <-
  comb_model |>
  lapply(
    lapply
  ...
  lapply(
    ...
    , mutate_at
    , "step"
    , \(x) ifelse(str_detect(x, "\\="), x, paste0(x, "="))
  ) |>
  lapply(lapply, separate, step, c("step", "pred"), sep = "\\=") |>
  lapply(
    lapply
    , left_join
    , select(cost_of_outcome, step, variable, cost = answer)
...
```{r Compute cost of single M-CHAT-R/F using study data, include=FALSE}
cost_of_single_m_chat_data <-
  m_chat_pred |>
  lapply(
    lapply
    , mutate
  ...
  imap(~ lapply(.x, mutate, step = str_remove_all(.y, "^single_"))) |>
  lapply(
    lapply
    , left_join
    , select(cost_of_outcome, step, variable, cost = answer)
...
```{r Compute weighted total cost of comb using study data, include=FALSE}
cost_of_pred_data <-
  cost_of_step_data |>
  c(cost_of_comb_data) |>
  c(cost_of_single_m_chat_data)

cost_of_pred_data <-
  names(cost_of_pred_data[[1]]) |>
  `names<-`(names(cost_of_pred_data[[1]])) |>
  lapply(
    \(x)
    names(cost_of_pred_data) |>
      `names<-`(names(cost_of_pred_data)) |>
      lapply(
        \(y)
        cost_of_pred_data[[y]][[x]]
      )
  ) |>
  lapply(reduce, rbind) |>
  lapply(mutate_at, "step", \(x) factor(x, unique(x))) |>
  lapply(group_by, step) |>
  lapply(summarize, avg_cost = sum(cost) / sum(n))
```

```{r Assess comb advantages - performance, include=FALSE}
comb_advantage_performance <-
  validation_performance |>
  mutate(Step = ifelse(Step == "", NA, Step)) |>
  fill(Step, .direction = "down") |>
  inner_join(
    selected_model$best_f1[
      ...
      rename_at(c("step", "algorithm"), str_to_sentence)
    , by = join_by(Step, Algorithm)
  ) |>
  select(-`AUC-ROC`) |>
  rbind(validation2_performance) |>
  gather(metric, estimate, -Step, -Algorithm) |>
  rename_all(str_to_lower) |>
  separate(estimate, c("avg", "lb", "ub", "leftover"), sep = " \\(|, |\\)") |>
  select(-leftover) |>
...
```{r Assess comb advantages - cost, include=FALSE}
comb_advantage_cost <-
  cost_of_pred_data$validation |>
  mutate(
    step =
      case_when(
        step == "neonatal_risk" ~ "Universal N"
        ...
        , step == "m_chat_r2f" ~ "Universal NBR/F"
        , step == "m_chat_rf" ~ "Universal NBRF"
        , step == "single_m_chat_r" ~ "Universal R"
        , step == "single_m_chat_f" ~ "Universal F"
        , step == "single_m_chat_r2f" ~ "Universal R/F"
        , step == "only_bsid_iii" ~ "Universal B"
        , TRUE ~ step
      )
  ) |>
  left_join(
    select(comb_advantage_performance, step, algorithm)
    , by = join_by(step)
  ) |>
  mutate(metric = "Avg. cost", avg = avg_cost, lb = NA, ub = NA) |>
  select(-avg_cost)
```

```{r Assess comb advantages - plot, include=FALSE}
comb_advantage <-
  comb_advantage_performance |>
  rbind(comb_advantage_cost) |>
  left_join(
    comb_model_protocol |>
      group_by(model) |>
      filter(step == max(step)) |>
      ungroup() |>
      select(step = model, max_step = value) |>
      mutate(
        max_step =
          case_when(
            max_step == "neonatal_risk" ~ "N"
            , max_step == "bsid_iii" ~ "NB"
            , max_step == "m_chat_r" ~ "NBR"
            , max_step == "m_chat_f" ~ "NBF"
            , max_step == "m_chat_r2f" ~ "NBR/F"
            , max_step == "m_chat_rf" ~ "NBRF"
            , max_step == "only_bsid_iii" ~ "B"
          )
      )
    , by = join_by(step)
  ) |>
  mutate(
    max_step =
      ifelse(is.na(max_step), str_remove_all(step, "Universal "), max_step)
  ) |>
  mutate(max_step = factor(max_step, unique(max_step))) |>
  mutate_at("step", \(x) factor(x, rev(unique(x)))) |>
  ggplot(aes(step, avg, color = max_step)) +
  geom_errorbar(aes(ymin = lb, ymax = ub), width = 0.1) +
  geom_point() +
  facet_grid(max_step ~ metric, scales = "free", space = "free_y") +
  coord_flip() +
  xlab("") +
  ylab("Average (95% CI)") +
  scale_color_discrete("Last step") +
  theme(
    strip.text.y.right = element_blank()
  )
```

```{r Assess comb advantages - show, echo=FALSE, fig.height=3.5, fig.width=10}
comb_advantage
```

```{r Assess comb advantages - caption, echo=FALSE}
figure_n <- figure_n + 1
cat(
  paste0(
    "Figure ", figure_n,". "
    , "ASD combined prediction comparison. ASD, autism spectrum disorder; CI, "
    , "confidence interval; Avg., average; BSID-III, Bayley scales of infant "
    , "development version III; CI, confidence interval; F1, 2 x precision x "
    , "recall / (precision + recall); FNR, false negative rate or specificity; "
    , "FPR, false positive rate or sensitivity/recall; M-CHAT-R, modified "
    , "checklist for autism in toddlers, revised; N, neonatal risk factors; "
    , "NBR, neonatal risk factors, BSID-III, and M-CHAT-R; NPV, negative "
    , "predictive value or precision; PPV, positive predictive value or "
    , "precision; TNR, true negative rate or specificity; TPR, true positive "
    , "rate or sensitivity/recall."
  )
)
```

```{r Select best combination, include=FALSE}
selected_comb <- c("N0-B0-NBR", "N0-B")
```

```{r Test AUC-ROCs, include=FALSE}
test_auroc <-
  eval$discrimination[
    names(selected_model$best_f1)[
      sapply(selected_model$best_f1, \(x) length(x) > 0)
  ...
  imap(~ .x[selected_model$best_f1[[.y]]]) |>
  lapply(imap, ~ mutate(.x$test$metrics, algorithm = .y)) |>
  lapply(reduce, rbind) |>
  imap(~ mutate(.x, step = .y)) |>
  reduce(rbind) |>
  select(step, algorithm, everything()) |>
...
```{r Test confusion matrix, include=FALSE}
test_cm <-
  eval$threshold[
    names(selected_model$best_f1)[
      sapply(selected_model$best_f1, \(x) length(x) > 0)
  ...
  imap(~ .x[selected_model$best_f1[[.y]]]) |>
  lapply(imap, ~ mutate(.x$test, algorithm = .y)) |>
  lapply(reduce, rbind) |>
  imap(~ mutate(.x, step = .y)) |>
  reduce(rbind) |>
  select(step, algorithm, th = ref_value, everything()) |>
...
```{r Test performances - create, include=FALSE}
test_performance <-
  test_auroc |>
  left_join(test_cm, by = join_by(step, algorithm)) |>
  select(-th, -nb) |>
  mutate(
    algorithm =
      factor(
...
test_performance <-
  test_performance |>
  rename_if(
    str_detect(colnames(test_performance), "step|algorithm")
    , str_to_sentence
  ) |>
  rename_if(
    !str_detect(colnames(test_performance), "step|algorithm")
...
```{r Test 2 confusion matrix, include=FALSE}
test2_cm <-
  eval2$threshold |>
  imap(
    ~ .x$test |>
...
```{r Test 2 performances - create, include=FALSE}
test2_performance <-
  test2_cm |>
  select(-th, -nb) |>
  mutate(
...
test2_performance <-
  test2_performance |>
  rename_if(
    str_detect(colnames(test2_performance), "step|algorithm")
    , str_to_sentence
  ) |>
  rename_if(
    !str_detect(colnames(test2_performance), "step|algorithm")
...
```{r Combine validation and test performance, include=FALSE}
eval_performance <-
  validation_performance |>
  filter(Step != "") |>
  select(-`AUC-ROC`) |>
  rbind(
    validation2_performance |>
      filter(
        Step
        %in% c(selected_comb, "Universal R", "Universal F", "Universal R/F")
      )
  ) |>
...
```{r Test performance - show, echo=FALSE}
table_n <- table_n + 1
...
eval_performance |>
  ...
  select(-Set) |>
  kable(
    caption = paste0("Table ", table_n, ". Test results (95% CI).")
    , format = kable_format
  ) |>
  footnote(
    paste0(
...
```{r Val-test diff in perform - create, include=FALSE}
diff_eval_performance <-
  eval_performance |>
  gather(metric, estimate, -Step, -Algorithm, -Set) |>
  filter(metric != "F1") |>
  rename_all(str_to_lower) |>
  separate(estimate, c("avg", "lb", "ub", "leftover"), sep = " \\(|, |\\)") |>
  select(-leftover) |>
  mutate_at(c("avg", "lb", "ub"), as.numeric) |>
  mutate(
    metric = factor(metric, c("F1", "TPR", "PPV", "TNR", "NPV"))
  ) |>
  mutate_at("set", \(x) factor(x, unique(x))) |>
  mutate_at("step", \(x) factor(x, rev(unique(x)))) |>
  ggplot(aes(step, avg, color = set)) +
  geom_errorbar(aes(ymin = lb, ymax = ub), width = 0.1) +
  geom_point() +
  ...
  xlab("") +
  scale_y_continuous(
    "Average (95% CI)", limits = c(0, 100), breaks = seq(0, 100, 10)
  ) +
...
```{r Val-test diff in perform - show, echo=FALSE, fig.height=3, fig.width=10}
diff_eval_performance
```

# Error analysis

```{r Best model prediction -write, eval=FALSE, include=FALSE}
eval2$df[[selected_comb[2]]] |>
  imap(
    ~ write_csv(.x, paste0("inst/extdata/best_model_prediction_", .y, ".csv"))
  )
```

# Model deployment

```{r Compute prevalence for observation and prediction, include=FALSE}
prevalence_obs_pred_step <-
  comb_step_pred |>
  lapply(gather, variable, value, -id) |>
  lapply(mutate_at, c("id", "variable"), \(x) factor(x, unique(x))) |>
  lapply(mutate_at, "value", factor) |>
  lapply(group_by, variable, value) |>
  lapply(summarize, n = n(), .groups = "drop") |>
  lapply(group_by, variable) |>
  lapply(mutate, total = sum(n)) |>
  lapply(ungroup) |>
  lapply(mutate, p = n / total) |>
  lapply(mutate, step = "universal") |>
  lapply(select, step, everything())
```

```{r Compute prevalence for prediction by combination, include=FALSE}
prevalence_pred_comb <-
  comb_model[selected_comb] |>
  lapply(lapply, select, -obs) |>
  lapply(lapply, gather, variable, value, -id, -step) |>
  lapply(lapply, mutate_at, c("id", "step"), \(x) factor(x, unique(x))) |>
  lapply(lapply, mutate_at, "value", factor) |>
  lapply(lapply, group_by, step, variable, value) |>
  lapply(lapply, summarize, n = n(), .groups = "drop") |>
  lapply(lapply, group_by, variable) |>
  lapply(lapply, mutate, total = sum(n)) |>
  lapply(lapply, ungroup) |>
  lapply(lapply, mutate, p = n / total) |>
  imap(~ lapply (.x, mutate, variable = .y))

prevalence_pred_comb <-
  names(prevalence_pred_comb[[1]]) |>
  `names<-`(names(prevalence_pred_comb[[1]])) |>
  lapply(
    \(x)
    names(prevalence_pred_comb) |>
      `names<-`(names(prevalence_pred_comb)) |>
      lapply(
        \(y)
        prevalence_pred_comb[[y]][[x]]
      )
  ) |>
  lapply(reduce, rbind)
...
prevalence_pred_comb <-
  prevalence_pred_comb |>
  lapply(mutate_at, "step", as.character) |>
  lapply(
    mutate, step = ifelse(str_detect(step, "\\="), step, paste0(step, "="))
  ) |>
  lapply(separate, step, c("step", "value2"), sep = "\\=") |>
  lapply(select, -value2) |>
  lapply(mutate_at, "variable", \(x) factor(x, unique(x))) |>
  lapply(mutate, step = factor(step, names(predictor$asd))) |>
  lapply(arrange, variable, step, value)
```

```{r Compute prevalence for prediction by single M-CHAT, include=FALSE}
prevalence_pred_single_m_chat <-
  m_chat_pred |>
  lapply(lapply, select, -obs) |>
  lapply(lapply, gather, variable, value, -id) |>
  lapply(lapply, mutate_at, "id", \(x) factor(x, unique(x))) |>
  lapply(lapply, mutate_at, "value", factor) |>
  lapply(lapply, group_by, variable, value) |>
  lapply(lapply, summarize, n = n(), .groups = "drop") |>
  lapply(lapply, group_by, variable) |>
  lapply(lapply, mutate, total = sum(n)) |>
  lapply(lapply, ungroup) |>
...
prevalence_pred_single_m_chat <-
  names(prevalence_pred_single_m_chat[[1]]) |>
  `names<-`(names(prevalence_pred_single_m_chat[[1]])) |>
  lapply(
    \(x)
    names(prevalence_pred_single_m_chat) |>
      `names<-`(names(prevalence_pred_single_m_chat)) |>
      lapply(
        \(y)
        prevalence_pred_single_m_chat[[y]][[x]]
      )
  ) |>
  lapply(reduce, rbind) |>
  lapply(mutate_at, "variable", \(x) factor(x, unique(x))) |>
...
```{r Combine computed prevalence for observation and prediction, include=FALSE}
prevalence_obs_pred <-
  prevalence_obs_pred_step |>
  imap(~ rbind(.x, prevalence_pred_comb[[.y]])) |>
  imap(~ rbind(.x, prevalence_pred_single_m_chat[[.y]])) |>
  lapply(
    mutate_at, "step", \(x) factor(x, c("universal", names(predictor$asd)))
  )
```

```{r Combined prevalence for obs and pred - create, include=FALSE}
prevalence_obs_pred_whole <-
  prevalence_obs_pred |>
  reduce(rbind)|>
  mutate(
    step =
      factor(
        step
        , c(setdiff(levels(step), "only_bsid_iii")[
              seq(which(setdiff(levels(step), "only_bsid_iii") == "bsid_iii"))
            ]
            , "only_bsid_iii"
            , setdiff(levels(step), "only_bsid_iii")[
                seq(
                  which(setdiff(levels(step), "only_bsid_iii") == "bsid_iii")
                  + 1
                  , length(setdiff(levels(step), "only_bsid_iii"))
                )
              ]
          )
      )
...
prevalence_obs_pred_whole <-
  prevalence_obs_pred_whole |>
  select(-total, -p) |>
  group_by(variable, step, value) |>
  summarize(n = sum(n), .groups = "drop") |>
  left_join(
    select(prevalence_obs_pred_whole, variable, total) |>
      unique() |>
      group_by(variable) |>
  ...
  mutate(
    value =
      ifelse(value == 1, "ASD (+) n (%)", "ASD (-) n (%)") |>
      factor(c("ASD (+) n (%)", "ASD (-) n (%)"))
  ) |>
  spread(value, n_p, fill = "0 (0)") |>
  mutate_at(
    c("variable", "step")
    , \(x)
      case_when(
        ...
        , x == "m_chat_r2f" ~ "Universal NBR/F"
        , x == "m_chat_rf" ~ "Universal NBRF"
        , x == "single_m_chat_r" ~ "Universal R"
        , x == "single_m_chat_f" ~ "Universal F"
        , x == "single_m_chat_r2f" ~ "Universal R/F"
        , x == "universal" ~ "N/A"
        , x == "only_bsid_iii" ~ "Universal B"
        , x == "obs" ~ "Observed outcome"
        , TRUE ~ x
      )
  ) |>
  group_by(variable) |>
  mutate(seq = seq(n())) |>
  ungroup() |>
  mutate(step = ifelse(seq == 1, step, str_remove_all(step, "Universal "))) |>
  select(-seq) |>
  mutate(variable = ifelse(duplicated(variable), "", variable))

prevalence_obs_pred_whole <-
  prevalence_obs_pred_whole |>
  rename_if(!str_detect(names(prevalence_obs_pred_whole), "ASD"), str_to_sentence)
```

```{r Combined prevalence for obs and pred - show, echo=FALSE}
table_n <- table_n + 1
...
prevalence_obs_pred_whole |>
  kable(
    caption =
      paste0(
        "Table ", table_n, ". "
        , "Prevalence for observed and predicted outcome in all sets (N = 470)."
      )
    , format = kable_format
  ) |>
  footnote(
...
```{r Workflow prevalence - define, include=FALSE}
workflow_prevalence <-
  read_csv("inst/extdata/workflow_prevalence.csv", show_col_types = FALSE)
```

```{r Workflow prevalence - draw, include=FALSE}
workflow_prevalence_dag <-
  unique(workflow_prevalence$variable) |>
  `names<-`(unique(workflow_prevalence$variable)) |>
  lapply(
    \(x)
    workflow_prevalence |>
      mutate_at(c("from", "to"), str_replace_all, " \\(", "\n(")
  ) |>
  imap(~ filter(.x, variable == .y)) |>
  lapply(graph_from_data_frame, directed = TRUE) |>
  lapply(
    \(x)
    ggnetwork(
      x
      , layout = layout_as_tree(x, circular = FALSE)
      , arrow.gap = 0.01
    )
  ) |>
  lapply(
    ...
    ggplot(x, aes(x, y, xend = xend, yend = yend)) +
      geom_blank(
        data =
          data.frame(x = c(0 - 0.3, 1 + 0.3)) |>
          mutate(y = x, xend = x, yend = x)
      ) +
      geom_edges(arrow = arrow(length = unit(0.5, "line"), type = "closed")) +
      geom_nodelabel(aes(label = name), size = 3, vjust = 1, hjust = 0.5) +
      theme_void()
  )
...
workflow_prevalence_dag <-
  ggarrange(
    ggarrange(
      workflow_prevalence_dag$`Universal N`
      , workflow_prevalence_dag$`Universal B`
      , nrow = 2, ncol = 1, labels = LETTERS[1:2]
    )
    , workflow_prevalence_dag$`N0-B`
    , nrow = 1, ncol = 2, widths = c(3, 3), labels = c("", LETTERS[3])
  )
```

```{r Workflow prevalence - show, echo=FALSE, fig.height=4, fig.width=6}
workflow_prevalence_dag
```

```{r Workflow prevalence - caption, echo=FALSE}
figure_n <- figure_n + 1
cat(
  paste0(
    "Figure ", figure_n,". "
    , "Workflow with prevalence: (A) Universal N; (B) Universal B; (C) N0-B. "
    , "BSID-III, Bayley scales of infant development version III; N, neonatal "
    , "risk factors; B, only BSID-III."
  )
)
```

```{r}
c(predictor$asd$neonatal_risk
  , predictor$asd$only_bsid_iii
  ) |>
  paste0(collapse = ", ") |>
  cat()
```
...
```{r}
predmod_data$asd$neonatal_risk |>
  reduce(rbind) |>
  select_at(predictor$asd$neonatal_risk) |>
  lapply(unique) |>
  lapply(sort) |>
  c(predmod_data$asd$only_bsid_iii |>
      reduce(rbind) |>
      select_at(predictor$asd$only_bsid_iii) |>
      lapply(unique) |>
      lapply(sort)
  )
```
...
predmod_data$asd$neonatal_risk |>
  ...
  summarize_all(mean) |>
  cbind(
    predmod_data$asd$only_bsid_iii |>
      reduce(rbind) |>
      select_at(predictor$asd$only_bsid_iii) |>
      summarize_all(mean)
  )
...
```{r Effect size plot in paper, include=FALSE}
paper_es_plot <-
  data.frame(variable_term = levels(es_plots[[1]]$data$variable_term)) |>
  separate(variable_term, c("variable", "term"), sep = " = ") |>
  pull(variable) |>
  str_replace_all("fine motor", "fine motor*") |>
  str_replace_all("receptive language", "receptive language*")
...
paper_es_plot <-
  ...
  left_join(
    data.frame(
      variable_term = 
        factor(
          levels(es_plots[[1]]$data$variable_term)
          , levels(es_plots[[1]]$data$variable_term)
        )
      , variable = factor(paper_es_plot, paper_es_plot)
    )
    , by = join_by(variable_term)
  ) |>
  mutate(variable_term = variable) |>
  select(-variable) |>
  ggplot(aes(variable_term, OR, color = adjustment)) +
  geom_hline(yintercept = 1, lty = 2) +
  geom_point(
  ...
  geom_errorbar(
    aes(ymin = LB, ymax = UB)
    , width = 0.75, na.rm = TRUE
  ) +
  coord_flip() +
  xlab("Predictor") +
  scale_y_continuous(
    "OR (95% CI)", limits = c(2^-1.1, 4)
    , breaks = c(2^-1, 2^0, 2^1, 2^2)
    , labels = c("0.5", "1", "2", "4+")
    , trans = "log2"
  ) +
...
```{r ROC curves in paper, include=FALSE}
paper_roc <-
  list(
    plot =
      list(
        data =
          ...
          imap(
            ~ mutate(.x$validation$plot$data, model = .y)
          ) |>
          reduce(rbind) |>
          mutate_at(
            "model"
            , \(x)
              factor(x, c(unique(x), "Neonatal risk 0 –\nBSID-III", "MCHAT-R"))
          )
      )
  ) |>
  arrange_disc_plot(multiple = TRUE)
  ...
  geom_point(
    data =
      ...
      inner_join(
        data.frame(
            th2 =
              c(`Neonatal risks` = th$neonatal_risk
                , `BSID-III` = th$only_bsid_iii
                , `NBR` = th$m_chat_r
              )
          ) |>
          rownames_to_column(var = "model")
        , by = join_by(th2, model)
      ) |>
      rbind(
        list(
            `MCHAT-R` =
              eval2$df$single_m_chat_r$validation
          ) |>
          imap(
            ~ mutate(.x, model = .y)
          ) |>
          ...
          group_by(obs, pred, model) |>
          summarize(n = n(), .groups = "drop") |>
          mutate_at(c("obs", "pred"), as.character) |>
          mutate_at(c("obs", "pred"), as.numeric) |>
          mutate(
            cm =
              case_when(
                obs == 1 & pred == 1 ~ "tp"
          ...
          select(model, cm, n) |>
          spread(cm, n) |>
          mutate(th2 = NA, tpr = tp / (tp + fn), tnr = tn / (tn + fp)) |>
          select(th2, tpr, tnr, model)
      ) |>
      mutate_at("model", \(x) factor(x, unique(x)))
    , size = 1
  ) +
...
```{r SHAP beeswarm plot in paper, include=FALSE}
paper_shap_beeswarm_plot <-
  shap_beeswarm_plots$m_chat_r[[selected_model$best_f1$m_chat_r]]$data |>
  pull(feature) |>
  levels() |>
  str_wrap(30)
...
paper_shap_beeswarm_plot <-
  ...
  left_join(
    data.frame(
        feature =
          shap_beeswarm_plots$m_chat_r[[
            selected_model$best_f1$m_chat_r
          ]]$data |>
          pull(feature) |>
          levels()
        , label = paper_shap_beeswarm_plot
      ) |>
      mutate_all(\(x) factor(x, x))
    , by = join_by(feature)
  ) |>
  mutate(feature = label) |>
  select(-label) |>
  shap_beeswarm_plotter(
    transparency = 1, text_size = 6, feature_rank_num = FALSE
  )
```

```{r Figure 2 in paper - create, include=FALSE}
paper_figure2 <-
  ggarrange(
    paper_es_plot
    , ggarrange(
        paper_roc
        , paper_shap_beeswarm_plot
        , nrow = 1, ncol = 2, widths = c(1.5, 3.6181)
        , labels = LETTERS[2:3], font.label = list(size = 9)
      )
    , nrow = 2, ncol = 1, heights = c(2, 2.5)
    , labels = c(LETTERS[1], ""), font.label = list(size = 9)
  )
```

```{r Figure 2 in paper - show, echo=FALSE, fig.height=4.5, fig.width=5.1181}
paper_figure2
```

```{r Figure 2 in paper - save, eval=FALSE, include=FALSE}
paper_figure2
ggsave("doc/figure2.eps", height = 4.5, width = 5.1181)
```
