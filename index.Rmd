...
workflow_prevalence_dag <-
  ggarrange(
    ggarrange(
      ...
      , workflow_prevalence_dag$`Universal NBR`
      , workflow_prevalence_dag$`Universal R`
      , nrow = 3, ncol = 1, labels = LETTERS[1:3]
    )
    , workflow_prevalence_dag$`N0-NBR`
    , nrow = 1, ncol = 2, widths = c(3, 4), labels = c("", LETTERS[4])
  )
```

```{r Workflow prevalence - show, echo=FALSE, fig.height=6, fig.width=7}
workflow_prevalence_dag
```
...
cat(
  paste0(
    "Figure ", figure_n,". "
    , "Workflow with prevalence: (A) Universal N; (B) Universal NBR; (C) "
    , "Universal R; and (D) N0-NBR. BSID-III, Bayley scales of infant "
    , "development version III; F, M-CHAT-R, modified checklist for autism in "
    , "toddlers, revised; N, neonatal risk factors; NBR, neonatal risk "
    , "factors, BSID-III, and M-CHAT-R; R, M-CHAT-R."
  )
)
```
