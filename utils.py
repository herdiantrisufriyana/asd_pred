...
def train_and_save_model(prefix, model_details, save_dir, cv=5, scoring='precision', task='classification', seed=42):
...
def load_and_predict(prefix, dataset, prediction_type='label', save_dir='default'):
    ...
    X_new = pd.read_csv(f'inst/extdata/{prefix}_{dataset}.csv')
    ...
    results_path = f'inst/extdata/{save_dir}/{dataset}_{filename}'
